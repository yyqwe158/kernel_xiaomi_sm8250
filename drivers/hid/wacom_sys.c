...
#define DEV_ATTR_RW_PERM (0664)
#define DEV_ATTR_WO_PERM (0220)
#define DEV_ATTR_RO_PERM (0440)
			    ...
			    size_t size, unsigned int retries)
{
	...
	if (retval < 0)
		hid_err(hdev, "wacom_get_report: ran out of retries (last error = %d)\n",
			retval);
			    ...
			    size_t size, unsigned int retries)
{
	...
	if (retval < 0)
		hid_err(hdev, "wacom_set_report: ran out of retries (last error = %d)\n",
			retval);
