...
static inline bool can_reclaim(short before_reclaim_adj, struct mm_struct *mm,
			       struct task_struct *task)
{
	short cur_oom_score_adj;

	if (!running_state || fatal_signal_pending(task) ||
	    task->flags & PF_EXITING || !list_empty(&mm->mmap_sem.wait_list)) {
	...
	if ((cur_oom_score_adj < CACHED_APP_MIN_ADJ &&
		...
		pr_info("[c:%s %d, r:%s %d] adj adjust %d %d\n", current->comm,
			current->pid, task->comm, task->pid, before_reclaim_adj,
			cur_oom_score_adj);
		return false;
	}
